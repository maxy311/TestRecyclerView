...
public class RecyclerView extends ViewGroup implements ScrollingView, NestedScrollingChild {
    ...
    private class SkipZone {
        public int mMinY;
        public int mMaxY;

        public SkipZone(int minY, int maxY) {
            mMinY = minY;
            mMaxY = maxY;
        }

        public boolean isNeedSkip(int targetY) {
            return targetY > mMinY && targetY < mMaxY;
        }
    }

    private static final int SCROLLTOP_JUMP_POSITION = 5;
    private SkipZone mSkipZone;
    ...
    public void setSkipZone(int minSkipZoneY, int maxSkipZoneY) {
        mSkipZone = new SkipZone(minSkipZoneY, maxSkipZoneY);
    }
    ...
    public void setLayoutManager(LayoutManager layout) {
        ...
        // We should do this switch a dispachLayout pass and animate children. There is a good
    ...
    public void smoothScrollToTop() {
        ...
        LinearLayoutManager layoutManager = (LinearLayoutManager)mLayout;
        if (layoutManager.findLastVisibleItemPosition() > SCROLLTOP_JUMP_POSITION) {
            scrollToPosition(SCROLLTOP_JUMP_POSITION);
            mCurrScrollY = 0;
        }
    ...
    protected void onMeasure(int widthSpec, int heightSpec) {
        if (mAdapterUpdateDuringMeasure) {
            ...
            if (mState.mRunPredictiveAnimations) {
                // try to provide a better approach.
    ...
    public void draw(Canvas c) {
        ...
        // If padding is not 0 and chilChildrenToPadding is false, to draw glows properly, we
    ...
    private class ViewFlinger implements Runnable {
        ...
        public void fling(int velocityX, int velocityY) {
            ...
            int finalScrollY = mCurrScrollY + mScroller.getFinalY();
            if (mSkipZone != null && mSkipZone.isNeedSkip(finalScrollY)) {
                while (mSkipZone.isNeedSkip(finalScrollY)) {
                    if (velocityY > 0)
                        velocityY += RecyclerView.this.getMaxFlingVelocity() / 200;
                    else
                        velocityY -= RecyclerView.this.getMaxFlingVelocity() / 200;

                    mScroller.abortAnimation(); // reset mScroller.
                    if (velocityY > 0)
                        mScroller.fling(0, 0, velocityX, velocityY,
                                Integer.MIN_VALUE, Integer.MAX_VALUE, Integer.MIN_VALUE, Integer.MAX_VALUE);
                    else
                    ...
                    finalScrollY = mCurrScrollY + mScroller.getFinalY();
                }

//                if (velocityY > 0)
//                    mScroller.fling(0, 0, velocityX, velocityY,
//                            Integer.MIN_VALUE, Integer.MAX_VALUE, Integer.MIN_VALUE, mSkipZone.mMaxY - mCurrScrollY);
//                else
//                    mScroller.fling(0, 0, velocityX, velocityY,
//                            Integer.MIN_VALUE, Integer.MAX_VALUE, - mCurrScrollY, Integer.MAX_VALUE);
            }
    ...
    private static class TraceCompat {
        public static void beginSection(String sectionName) {
            try {
                 android.support.v4.os.TraceCompat.beginSection(sectionName);
                 //remedy Exception to Throwable to catch NoSuchMethodError on Custom rooted ROM on Samsung Galaxy NoteIII.
                 //ref: https://code.google.com/p/android/issues/detail?id=175222
            } catch (Throwable e) {}
        }
